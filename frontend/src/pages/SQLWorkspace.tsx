...
import { Play, Save, Download, Database, Table as TableIcon, Columns, Key, ChevronRight, ChevronDown, Loader2, Trash, Wand2, Sparkles, Wrench, AlertTriangle } from 'lucide-react';
...
import { isBroadDestructiveStatement } from '@/lib/sqlSafety';
...
export default function SQLWorkspace() {
  ...
  // AI-generated SQL from the chat sidebar's "Execute" action — held here
  // instead of running immediately, so it can be reviewed in a
  // ConfirmModal first. Every other consequential action in this app goes
  // through that same confirm step; this was the one that didn't.
  const [pendingAiSql, setPendingAiSql] = useState<string | null>(null);
  ...
  return (
    <div className="flex h-full flex-col">
              ...
              <Panel defaultSize="25%" minSize="15%" maxSize="40%" id="ai-chat">
                <div className="h-full w-full bg-canvas overflow-hidden flex flex-col">
                  <AIChatSidebar 
                    ...
                    onExecuteQuery={(sql) => setPendingAiSql(sql)}
      ...
      <ConfirmModal
        isOpen={!!pendingAiSql}
        onClose={() => setPendingAiSql(null)}
        onConfirm={() => {
          if (!pendingAiSql) return;
          setQuery(pendingAiSql);
          executeMutation.mutate(pendingAiSql);
        }}
        title="Run AI-generated SQL?"
        message={
          <div className="flex flex-col gap-3">
            {pendingAiSql && isBroadDestructiveStatement(pendingAiSql) && (
              <div className="flex items-start gap-2 bg-red-600/15 border border-red-600/40 rounded p-2 text-xs text-red-500">
                <AlertTriangle size={14} className="shrink-0 mt-0.5" />
                <span>This statement has no WHERE clause — it will affect every row in the table.</span>
              </div>
            )}
            <p>This SQL was generated by AI, not typed by you — review it before running it against your database:</p>
            <pre className="bg-canvas-night border border-border rounded p-3 text-xs font-mono text-foreground overflow-x-auto whitespace-pre-wrap max-h-48 overflow-y-auto">
              {pendingAiSql}
            </pre>
          </div>
        }
        ...
        cancelText="Cancel"
        isDestructive={!!pendingAiSql && isBroadDestructiveStatement(pendingAiSql)}
      />
    </div>
  );
}
